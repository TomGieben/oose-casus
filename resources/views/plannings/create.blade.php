...
<div class="container my-5">
    <h2 class="text-center mb-4">{{ __('Add Planning') }}</h2>
    ...
    <form action="{{ route('plannings.store') }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="time" class="form-control" id="starts_at" name="starts_at" value="{{ old('starts_at') }}">
        </div>

        <div class="mb-3">
            <label for="ends_at" class="form-label">{{ __('Ends At') }}</label>
            <input type="time" class="form-control" id="ends_at" name="ends_at" value="{{ old('ends_at') }}">
        </div>
